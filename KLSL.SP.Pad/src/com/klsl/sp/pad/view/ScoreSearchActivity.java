...
import com.klsl.sp.pad.adapter.SearchScoreSimpleAdapter;
import com.klsl.sp.pad.dao.Dao;
import com.klsl.sp.pad.utility.CustomDict;
...
public class ScoreSearchActivity extends Activity {
		...
		try {
			...
			if (cursor.getCount() > 0) {
				...
				implementation.setText(cursor.getString(cursor
						.getColumnIndex("AdditionalDesc")));
				getImageList(projectCode);
			}
			else
			{
	...
	private void getImageList(String projectCode) {
		try {
			Cursor cursor = _dao.SearchImageList(projectCode);
			if (cursor.getCount() > 0) {
				ListView listview = (ListView) findViewById(R.id.listImage);
				List<HashMap<String, String>> data = new ArrayList<HashMap<String, String>>();
